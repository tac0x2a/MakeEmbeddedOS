...
# モトローラSレコードフォーマット
* ASCII文字で構成されているので7bitあれば転送できる
* フォーマットが単純なのでローダの実装が楽
* 行単位に展開先情報をもつため、ダウンロードしながら展開するのに向いている
...
## CPU
比較器やマルチプレクサを組み合わせて、命令に対応する回路だけを動作させるためのCPU回路をCPUの``コア``という。命令セットはコアによって決まる。
コアに対して、周辺のI/Oコントローラを``ペリフェラル``という。
